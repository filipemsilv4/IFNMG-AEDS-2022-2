...
No * cab_linha(Matriz * ap_mat, int nlin){
  ...
  while(atual_linha!= ap_mat -> cabeca && atual_linha->linha <=nlin){   // ERRO
...
No * cab_coluna(Matriz * ap_mat, int ncol){
  ...
  while(atual_coluna!= ap_mat -> cabeca && atual_coluna->coluna <=ncol){  // ERRO
...
//No* atual_coluna = cab_col -> abaixo;
  ...
  while(atual_linha->coluna <= col && atual_linha != cab_lin){    // ERRO
    atual_linha = atual_linha ->  direita;
  }
  printf("teste %d", atual_linha -> coluna);
